...
const fs = require("fs");
const ayar = JSON.parse(
  fs.readFileSync("./ayarlar.json", { encoding: "utf-8" })
);
...
Voice.on("ready", async () => {
  ...
  const Guild =
    Voice.guilds.cache.get(ayar.ayarlar.SunucuID) || Voice.guilds.cache.first();
  if (!Guild) {
  ...
  Channel.join()
    ...
    .catch(err => {
      Voice.error(
        `Cannot connect to voice channel (${Channel.name}) (${Channel.id}): ` +
          err.message
      );
      return Voice.destroy();
    });
...
function playVoice(Voice) {
  try {
    const Path =
      Voice.staffJoined === true
        ? "./" + ayar.Ses.YetkiliGiris
        : "./" + ayar.Ses.Kayıtsızgiris;
...
Client.prototype.log = function(content) {
  return console.log(
    `[${moment().format("YYYY-MM-DD HH:mm:ss")}] [VOICE BOT] ${content}`
  );
};

Client.prototype.error = function(content) {
  return console.error(
    `[${moment().format("YYYY-MM-DD HH:mm:ss")}] [VOICE BOT] ERR! ${content}`
  );
};
    ...
    this.members.some(
      m =>
        ...
        m.roles.highest.position >=
          m.guild.roles.cache.get(ayar.ayarlar.EnAltYetkili).position
    )
  )
    return true;
  return false;
  ...
  if (
    ...
    ([...ayar.ayarlar.OwnerID].includes(this.id) ||
      ...
      this.roles.highest.position >=
        this.guild.roles.cache.get(ayar.ayarlar.EnAltYetkili).position)
  ) 
    return true;
  return false;
}; 
